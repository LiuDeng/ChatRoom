...
- (void)setupView {
    ...
    // UIButtons.
    [self.leftButton setBackgroundColor:[UIColor primaryBackgroundColor]];
    [self.rightButton setBackgroundColor:[UIColor primaryBackgroundColor]];
    self.leftButton.layer.masksToBounds = YES;
    self.leftButton.layer.cornerRadius = 2;
    self.rightButton.layer.masksToBounds = YES;
    self.rightButton.layer.cornerRadius = 2;
    
    [self.leftButton setTitle:@"登  录" forState:UIControlStateNormal];
    [self.rightButton setTitle:@"注  册" forState:UIControlStateNormal];
...
- (void)addAllConstraints {
    ...
    CGPoint center = self.overlayTitle.center;
    center.x = self.view.center.x;
    self.overlayTitle.center = center;
    ...
    [constraints addObject:[NSString stringWithFormat:@"H:|-%f-[_pageControl(==40)]", [UIScreen mainScreen].bounds.size.width / 2. - 20]];

    // GradientView.
    [self.gradientView setTranslatesAutoresizingMaskIntoConstraints:NO];
    [constraints addObject:@"V:[_gradientView(==200)]-0-|"];
    [constraints addObject:[NSString stringWithFormat:@"H:|-0-[_gradientView(==%f)]-0-|", [UIScreen mainScreen].bounds.size.width]];
